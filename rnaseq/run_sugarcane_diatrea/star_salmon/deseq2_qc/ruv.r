...
metadata <- read.table("../../../../raw_reads/sugarcane_diatrea_metadata_sugarcane.csv", header = TRUE, sep = ",")
# RUVr
# some magic
...
sample_files = paste0("/home/diegoj/rnaseq_diatraea/rnaseq/run_sugarcane_diatrea/star_salmon/", pull(metadata , "sample"), "/quant.sf")
# name table columns
names(sample_files) = pull(metadata, "sample")
# relate genes to transcripts
tx2gene = read.table("/home/diegoj/rnaseq_diatraea/rnaseq/run_sugarcane_diatrea/star_salmon/tx2gene.tsv", header = T)
# GENE MODE
...
dea_contrast <- results(dea, lfcThreshold= 1, altHypothesis="greaterAbs", parallel = T, contrast = c("Group", "Stem_120h_Diatrea", "Stem_120h_Diatrea+Fusarium"))
...
baseMeanA <- rowMeans(counts(dea, normalized=TRUE)[,colData(dea)$Group == "Stem_120h_Diatrea"])
baseMeanB <- rowMeans(counts(dea, normalized=TRUE)[,colData(dea)$Group == "Stem_120h_Diatrea+Fusarium"])

res = cbind(baseMeanA, baseMeanB, dea_df)
res = cbind(sampleA="Stem_120h_Diatrea", sampleB="Stem_120h_Diatrea+Fusarium", as.data.frame(res))
...
library(ggplot2)
library(dplyr)
library(parallel)colnames(GO) <- c("Gene", "GO_term")

#library(clusterProfiler)
numCores <- 150

GO <- read.table("/home/diegoj/rnaseq_diatraea/reference_genomes/sugarcane/annotation/go_table_locus_name.txt", header=FALSE, stringsAsFactors=FALSE, colClasses = c("character", "character"))
