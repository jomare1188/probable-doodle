...
kos_genes <- annot_df %>%
  select(gene = locusName, KO) %>%  # keep only the relevant columns
  filter(KO != "" & !is.na(KO)) %>% # remove empty or NA KO entries
...
read_go_data <- function(annot_file, go_results_file) {
  ...
  annot_df <- read.delim(annot_file)
  all_go <- annot_df %>%
  select(gene = locusName, GO = GO) %>%  # keep only the relevant columns
  filter(GO != "" & !is.na(GO)) %>% # remove empty or NA KO entries
...
run_enrichment_pipeline <- function(config) {
  ...
  up_kos <- kos_genes %>%
  dplyr::filter(gene %in% deg_data$up) %>%
  dplyr::pull(KO)


  down_kos <- kos_genes %>%
  dplyr::filter(gene %in% deg_data$down) %>%
  dplyr::pull(KO)

  
  universe_kos <- unique(kos)
...
config <- list(
  # Annotation files
  annot_file = "~/rnaseq_diatraea/reference_genomes/sugarcane/annotation/SofficinarumxspontaneumR570_771_v2.1.annotation_info.txt",
  ...
  # GO files
#  go_annotation_file = "~/rnaseq_diatraea/panzzer/annot_01/gene_go_annotations.txt",
